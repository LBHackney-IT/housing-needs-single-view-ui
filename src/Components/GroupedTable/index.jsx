...
export default class GroupedTable extends Component {
  ...
  constructor(props) {
    super(props);
    this.state = {selected: {}, groupedRecords: this.groupRecords(props.records)};
  }
  ...
  divider(name){
    return <tr key={name}><td colSpan="5" className="subHeader">{name}</td></tr>
  }
            ...
            { Object.keys(this.state.groupedRecords).map( source => {
              return [this.divider(source)].concat(
                this.state.groupedRecords[source].map(record => {
                  return <ResultRow key={record.id} result={record} selectable={this.props.selectable} onSelected={this.rowSelected} onDeselected={this.rowDeselected}/>
                })
              )
            }) }
          </tbody>
